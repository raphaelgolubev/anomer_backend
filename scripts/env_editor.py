...
class EnvFileEditor:
    ...
    def _should_quote(self, value: str) -> bool:
        ...
        return not (self._is_numeric(value) or self._is_boolean(value) or self._is_list(value))
    
    def _validate_param_name(self, param_name: str) -> None:
        """Проверяет, что параметр имеет префикс секции"""
        if '_' not in param_name:
            raise ValueError(f"Все параметры должны иметь префикс секции, например 'SERVER_DEBUG', где 'SERVER_' это префикс с названием секции")
    ...
    def add_param_to_section(self, section_name: str, param_name: str, param_value: str, description: str = None):
        ...
        if exists_param_index != -1:
            ...
            if '<' in self.lines[exists_param_index - 1] and '>' in self.lines[exists_param_index - 1]: 
        ...
        if description:
            ...
            if self._should_quote(param_value):
                comment_value = f'"{param_value}"' if not param_value.startswith('"') else param_value
            else:
        ...
        # Добавляем параметр
        if self._should_quote(param_value):
            param_value_formatted = f'"{param_value}"' if not param_value.startswith('"') else param_value
        else:
            ...
            if line.strip() and not line.strip().startswith('#'):
                ...
                if match:
                    ...
                    if param_name.startswith(old_prefix):
                        ...
                        line_ending = '\n' if line.endswith('\n') else ''
                        self.lines[i] = f'{indent}{new_param_name}={param_value}{line_ending}'
                        renamed_count += 1
        
        print(f"Секция '{old_name}' переименована в '{new_name}'")
        if renamed_count > 0:
            print(f"Переименовано {renamed_count} параметров с префиксом {old_prefix} на {new_prefix}")
    
    def remove_param(self, param_name: str):
        """Удаляет параметр из файла"""
        param_index = self.find_param(param_name)
        if param_index != -1:
            if '<' in self.lines[param_index - 1] and '>' in self.lines[param_index - 1]:
...
def main():
    parser = argparse.ArgumentParser(description='CLI инструмент для редактирования .env файлов')
    subparsers = parser.add_subparsers(dest='command', help='Доступные команды')
    
    # Команда add param
    add_param_parser = subparsers.add_parser('add', help='Добавить параметр или секцию')
    add_param_parser.add_argument('type', choices=['param', 'section'], help='Тип добавляемого элемента')
    ...
    add_param_parser.add_argument('-s', '--section', help='Секция для добавления параметра (определяется автоматически по префиксу)')
    ...
    remove_parser.add_argument('type', choices=['param', 'section'], help='Тип удаляемого элемента')
        ...
        if args.command == 'add':
            if args.type == 'param':
                ...
                if args.section:
                    ...
                    editor.add_param_to_section(args.section, args.name, args.value, args.description)
                else:
