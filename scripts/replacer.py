...
def main():
    ...
    parser = argparse.ArgumentParser(description='Заменяет *{VAR} в файле значениями из .env')
    ...
    parser.add_argument('-o', '--output', type=Path, help='путь для сохранения результата')
    parser.add_argument('--inplace', action='store_true', help='перезаписать целевой файл')
    args = parser.parse_args()

    # Проверка конфликтующих опций
    if args.inplace and args.output:
        print("Ошибка: указаны и --inplace, и -o/--output; выберите только один вариант.", file=sys.stderr)
        sys.exit(1)
